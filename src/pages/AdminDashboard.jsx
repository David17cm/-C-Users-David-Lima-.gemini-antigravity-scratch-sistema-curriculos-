...
export default function AdminDashboard() {
    ...
    // Helper para notificações premium
    ...
    const confirmDeleteVaga = async () => {
        ...
        try {
            ...
            notify('Vaga e suas dependências foram excluídas!', 'success');
        } catch (err) {
    ...
    const confirmDeleteUser = async () => {
        ...
        try {
            if (userToDelete.is_invite) {
                // É um convite pendente. Basta deletar da tabela empresa_invites
            ...
            notify('Registro excluído do sistema com sucesso!', 'success');
        } catch (err) {
    ...
    const handleCreateEmpresa = async (e) => {
        ...
        try {
            // 1. Criar o "convite/acesso prévio" na tabela empresa_invites
            ...
            // O registro em 'empresas' será criado automaticamente na AuthPage 
            // no momento do primeiro login, para garantir o vínculo correto com o Auth.id.
    ...
    const statCards = [
        ...
        { label: 'Usuários Totais', value: stats.usuariosAtivos, gradient: 'linear-gradient(135deg, #f59e0b 0%, #d97706 100%)', icon: <Shield size={24} color="#fff" /> },
    ];

    const menuItems = [
        { key: 'overview', label: 'Monitoramento', icon: <LayoutDashboard size={20} /> },
        { key: 'usuarios', label: 'Gestão de Usuários', icon: <Users size={20} /> },
        { key: 'empresas', label: 'Parceiros / Empresas', icon: <Building size={20} /> },
        { key: 'vagas', label: 'Curadoria de Vagas', icon: <Briefcase size={20} /> },
        { key: 'metricas', label: 'Inteligência de Dados', icon: <BarChart2 size={20} /> },
        { key: 'logs', label: 'Log de Auditoria', icon: <Clock size={20} /> },
        { key: 'lgpd', label: 'Privacidade LGPD', icon: <Database size={20} /> },
        { key: 'denuncias', label: 'Ouvidoria / Denúncias', icon: <AlertTriangle size={20} /> },
    ];
    ...
    return (
        <div style={{ display: 'flex', minHeight: '100vh', background: '#08080a', color: '#f8fafc', fontFamily: "'Outfit', sans-serif" }}>
            ...
            <main className="admin-main" style={{ 
                        ...
                        <div className="glass-panel admin-card" style={{ padding: '3rem', border: '1px solid rgba(124, 58, 237, 0.15)', background: 'linear-gradient(135deg, rgba(124, 58, 237, 0.03), rgba(0, 0, 0, 0))', marginBottom: '4rem' }}>
                            <div className="admin-section-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2.5rem' }}>
                                <div style={{ display: 'flex', alignItems: 'center', gap: '15px' }}>
                                    ...
                                    <div>
                                        ...
                                        <p style={{ margin: '4px 0 0', color: '#94a3b8', fontSize: '0.9rem' }}>Acompanhamento diário de novos usuários no sistema.</p>
                                    </div>
                                </div>
                            ...
                            {chartPeriod === 'custom' && (
                                <div style={{ display: 'flex', gap: '20px', marginBottom: '2rem', animation: 'fadeIn 0.3s ease-out', background: 'rgba(255,255,255,0.02)', padding: '15px', borderRadius: '12px', border: '1px solid rgba(255,255,255,0.05)' }}>
                                    ...
                                    <div style={{ flex: 1 }}>
                                        <label style={{ fontSize: '0.7rem', color: '#64748b', marginBottom: '6px', display: 'block', fontWeight: 800 }}>ATÉ:</label>
                                        <input 
                                            type="date" 
                        ...
                        <div className="glass-panel" style={{ padding: '3rem', border: '1px solid rgba(124, 58, 237, 0.15)', background: 'linear-gradient(135deg, rgba(124, 58, 237, 0.03), rgba(0, 0, 0, 0))' }}>
                            ...
                            <form onSubmit={handleInvite} style={{ display: 'flex', flexWrap: 'wrap', gap: '1.5rem', alignItems: 'end' }}>
                                <div className="input-group" style={{ marginBottom: 0 }}>
                                    <label style={{ fontSize: '0.75rem', fontWeight: 800, color: '#94a3b8', marginBottom: '12px', display: 'block', letterSpacing: '1px' }}>ENDEREÇO DE E-MAIL</label>
                            ...
                            <div className="table-responsive"><table style={{ width: '100%', borderCollapse: 'collapse' }}>
                                <thead style={{ background: 'rgba(255,255,255,0.02)', borderBottom: '1px solid rgba(255,255,255,0.08)' }}>
                                    <tr>
                                        ...
                                        <th style={{ padding: '1.5rem 2.5rem', textAlign: 'left', fontSize: '0.8rem', fontWeight: 800, color: '#94a3b8', letterSpacing: '1px' }}>AÇÕES</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    {allEmpresas.map(emp => (
                                        <tr key={emp.id} className="admin-table-row" style={{ borderBottom: '1px solid rgba(255,255,255,0.03)' }}>
                                            ...
                                            <td style={{ padding: '1.5rem 2.5rem', color: '#94a3b8', fontWeight: 600 }}>{emp.cnpj || '—'}</td>
                                            <td style={{ padding: '1.5rem 2.5rem', color: '#94a3b8', fontWeight: 600 }}>{emp.email_contato || '—'}</td>
                                            <td style={{ padding: '1.5rem 2.5rem' }}>
                                                <span style={{ 
                                                    padding: '6px 14px', 
                                                    borderRadius: '20px', 
                                        ...
                                        <div style={{ padding: '1.5rem', background: 'rgba(124, 58, 237, 0.05)', borderRadius: '16px', border: '1px solid rgba(124, 58, 237, 0.1)', marginBottom: '2rem' }}>
                                            ...
                                            <div className="input-group" style={{ marginBottom: 0 }}>
                                                ...
                                                <div style={{ position: 'relative' }}>
                                                    ...
                                                    <input type="password" title="Defina a senha que a empresa usará para o primeiro login" className="neon-input" required style={{ paddingLeft: '40px' }} value={newEmpresaData.password} onChange={e => setNewEmpresaData({...newEmpresaData, password: e.target.value})} placeholder="••••••••" />
                                                </div>
                                            </div>
                                        </div>
                    ...
                    <div className="glass-panel" style={{ padding: 0, overflow: 'hidden', animation: 'fadeIn 0.4s ease-out', border: '1px solid rgba(255,255,255,0.06)' }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '2.5rem', borderBottom: '1px solid rgba(255,255,255,0.05)', flexWrap: 'wrap', gap: '1.5rem' }}>
                            ...
                            <button onClick={handleExportarCSV} className="neon-button secondary" style={{ 
                                ...
                                <Download size={18} style={{ marginRight: '10px' }} /> RELATÓRIO COMPLETO (CSV)
                            </button>
                        </div>
                        <div style={{ overflowX: 'auto' }}>
                            <div className="table-responsive"><table style={{ width: '100%', borderCollapse: 'collapse' }}>
                                <thead style={{ background: 'rgba(255,255,255,0.01)', textAlign: 'left' }}>
                                    <tr>
                                        <th style={{ padding: '1.5rem 2.5rem', fontSize: '0.75rem', color: '#64748b', fontWeight: 800, letterSpacing: '1px' }}>DADOS ACADÊMICOS / NOME</th>
                                        <th style={{ padding: '1.5rem 2.5rem', fontSize: '0.75rem', color: '#64748b', fontWeight: 800, letterSpacing: '1px' }}>CONTATO E COMUNICAÇÃO</th>
                                        <th style={{ padding: '1.5rem 2.5rem', fontSize: '0.75rem', color: '#64748b', fontWeight: 800, letterSpacing: '1px' }}>CLASSIFICAÇÃO</th>
                                        <th style={{ padding: '1.5rem 2.5rem', fontSize: '0.75rem', color: '#64748b', fontWeight: 800, letterSpacing: '1px' }}>DATA CADASTRO</th>
                                        <th style={{ padding: '1.5rem 2.5rem', fontSize: '0.75rem', color: '#64748b', fontWeight: 800, letterSpacing: '1px' }}>AÇÕES</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    {usuarios
                ...
                {/* Restantes das abas seguem o padrão com cores melhoradas */}
                {activeTab === 'empresas' && (
                    <div className="glass-panel" style={{ padding: 0, overflow: 'hidden', animation: 'fadeIn 0.4s ease-out' }}>
                        ...
                        <div style={{ overflowX: 'auto' }}>
                            <div className="table-responsive"><table style={{ width: '100%', borderCollapse: 'collapse' }}>
                                <thead style={{ background: 'rgba(255,255,255,0.01)', textAlign: 'left' }}>
                                    <tr>
                                        <th style={{ padding: '1.5rem 2.5rem', fontSize: '0.75rem', color: '#64748b', fontWeight: 800 }}>IDENTIFICAÇÃO</th>
                                        <th style={{ padding: '1.5rem 2.5rem', fontSize: '0.75rem', color: '#64748b', fontWeight: 800 }}>SITUAÇÃO</th>
                                        <th style={{ padding: '1.5rem 2.5rem', fontSize: '0.75rem', color: '#64748b', fontWeight: 800 }}>MODERAÇÃO</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    {empresas.map(e => (
                                        <tr key={e.id} className="admin-table-row" style={{ borderBottom: '1px solid rgba(255,255,255,0.03)' }}>
                                            ...
                                            <td style={{ padding: '1.5rem 2.5rem' }}>
                                                <div style={{ display: 'flex', alignItems: 'center', gap: '8px', color: e.aprovada ? '#4ade80' : '#fbbf24', fontWeight: 700, fontSize: '0.85rem' }}>
                                                    ...
                                                    {e.aprovada ? 'ATIVO NO SISTEMA' : 'AGUARDANDO APROVAÇÃO'}
                                                </div>
                                            </td>
                                            <td style={{ padding: '1.5rem 2.5rem', display: 'flex', gap: '8px' }}>
                ...
                {activeTab === 'vagas' && (
                    <div className="glass-panel" style={{ padding: 0, overflow: 'hidden', animation: 'fadeIn 0.4s ease-out' }}>
                        <div style={{ padding: '2.5rem', borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
                            <h3 style={{ margin: 0, fontSize: '1.4rem', fontWeight: 800 }}>CONTROLE DE PUBLICAÇÕES</h3>
                        </div>
                        <div style={{ overflowX: 'auto' }}>
                            <div className="table-responsive"><table style={{ width: '100%', borderCollapse: 'collapse' }}>
                                <thead style={{ background: 'rgba(255,255,255,0.01)', textAlign: 'left' }}>
                                    <tr>
                                        <th style={{ padding: '1.5rem 2.5rem', fontSize: '0.75rem', color: '#64748b', fontWeight: 800 }}>TÍTULO DA VAGA / CONTRATANTE</th>
                                        <th style={{ padding: '1.5rem 2.5rem', fontSize: '0.75rem', color: '#64748b', fontWeight: 800 }}>STATUS ATUAL</th>
                                        <th style={{ padding: '1.5rem 2.5rem', fontSize: '0.75rem', color: '#64748b', fontWeight: 800 }}> GESTÃO</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    {vagas.map(v => (
                                            ...
                                            <td style={{ padding: '1.5rem 2.5rem' }}>
                                                <div style={{ display: 'flex', gap: '10px' }}>
                                                    <button onClick={() => handleFecharVaga(v)} disabled={v.status === 'encerrada'} 
                                                        ...
                                                        {v.status === 'encerrada' ? 'VAGA ENCERRADA' : 'CANCELAR PUBLICAÇÃO'}
                                                    </button>
                    ...
                    <div style={{ display: 'grid', gap: '2.5rem', animation: 'fadeIn 0.4s ease-out' }}>
                        <div className="glass-panel admin-card" style={{ padding: '3.5rem' }}>
                            ...
                            {metrics.vagasMaisProcuradas.length === 0 ? (
                                <p style={{ color: '#94a3b8', textAlign: 'center', padding: '3rem' }}>Dados insuficientes para gerar métricas.</p>
                            ) : (
                ...
                {/* LOGS, LGPD e DENÚNCIAS seguem o mesmo padrão refinado */}
                {activeTab === 'logs' && (
                    <div className="glass-panel" style={{ padding: 0, overflow: 'hidden', animation: 'fadeIn 0.4s ease-out' }}>
                         <div style={{ padding: '2.5rem', borderBottom: '1px solid rgba(255,255,255,0.05)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                            ...
                            <div style={{ padding: '6px 14px', borderRadius: '8px', background: 'rgba(255,255,255,0.02)', color: '#94a3b8', fontSize: '0.8rem', fontWeight: 700 }}>ÚLTIMOS 100 EVENTOS</div>
                        </div>
                        <div style={{ overflowX: 'auto', maxHeight: '600px' }}>
                            <div className="table-responsive"><table style={{ width: '100%', borderCollapse: 'collapse' }}>
                                <thead style={{ position: 'sticky', top: 0, background: '#0f0f14', zIndex: 10 }}>
                                    <tr style={{ textAlign: 'left' }}>
                                        <th style={{ padding: '1.5rem 2.5rem', fontSize: '0.75rem', color: '#64748b', fontWeight: 800 }}>USUÁRIO</th>
                                        <th style={{ padding: '1.5rem 2.5rem', fontSize: '0.75rem', color: '#64748b', fontWeight: 800 }}>AÇÃO SISTÊMICA</th>
                ...
                {activeTab === 'denuncias' && (
                    <div className="glass-panel" style={{ padding: 0, overflow: 'hidden', animation: 'fadeIn 0.4s ease-out' }}>
                        <div style={{ padding: '2.5rem', borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
                            <h3 style={{ margin: 0, fontSize: '1.4rem', fontWeight: 800, color: '#f87171' }}>TRIAGEM DE DENÚNCIAS</h3>
                        </div>
                        <div style={{ overflowX: 'auto' }}>
                            <div className="table-responsive"><table style={{ width: '100%', borderCollapse: 'collapse' }}>
                                <thead style={{ background: 'rgba(255,255,255,0.01)', textAlign: 'left' }}>
                                    <tr>
                                        ...
                                        <th style={{ padding: '1.5rem 2.5rem', fontSize: '0.75rem', color: '#64748b', fontWeight: 800 }}> INTERVENÇÃO</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    {denuncias.length === 0 ? (
                                        <tr><td colSpan="3" style={{ padding: '5rem', textAlign: 'center', color: '#64748b', fontWeight: 600 }}>Nenhum alerta pendente. Operação estável.✨</td></tr>
                                    ) : (
                                        denuncias.map(d => (
            ...
            {vagaToClose && (
                <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.85)', backdropFilter: 'blur(8px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 9999, animation: 'fadeIn 0.2s ease-out' }}>
                    <div className="glass-panel" style={{ textAlign: 'center', padding: '4rem', maxWidth: '500px', border: '1px solid rgba(239, 68, 68, 0.3)' }}>
                        ...
                        <h3 style={{ fontSize: '1.8rem', fontWeight: 900, marginBottom: '1.5rem', letterSpacing: '-0.5px' }}>ENCERRAR PUBLICAÇÃO?</h3>
                        <p style={{ color: '#94a3b8', marginBottom: '3rem', lineHeight: 1.6, fontSize: '1rem' }}>Esta vaga será ocultada para todos os candidatos imediatamento. Você poderá reverter esta ação nos logs se necessário.</p>
            ...
            {vagaToDelete && (
                <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.85)', backdropFilter: 'blur(8px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 9999, animation: 'fadeIn 0.2s ease-out' }}>
                    <div className="glass-panel" style={{ textAlign: 'center', padding: '4rem', maxWidth: '500px', border: '1px solid rgba(239, 68, 68, 0.3)' }}>
                        ...
                        <h3 style={{ fontSize: '1.8rem', fontWeight: 900, marginBottom: '1.5rem', letterSpacing: '-0.5px', color: '#f87171' }}>EXCLUSÃO IRREVERSÍVEL</h3>
                        <p style={{ color: '#94a3b8', marginBottom: '3rem', lineHeight: 1.6, fontSize: '1rem' }}>Você está prestes a excluir todos os registros da vaga <strong>{vagaToDelete.titulo}</strong>. Isso apagará todas as candidaturas e currículos atrelados a ela em definitivo.</p>
                        <div style={{ display: 'flex', gap: '1.5rem', justifyContent: 'center' }}>
            ...
            {userToDelete && (
                <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.85)', backdropFilter: 'blur(8px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 9999, animation: 'fadeIn 0.2s ease-out' }}>
                    <div className="glass-panel" style={{ textAlign: 'center', padding: '4rem', maxWidth: '500px', border: '1px solid rgba(239, 68, 68, 0.3)' }}>
                        ...
                        <h3 style={{ fontSize: '1.8rem', fontWeight: 900, marginBottom: '1.5rem', letterSpacing: '-0.5px', color: '#f87171' }}>EXCLUIR USUÁRIO?</h3>
                        <p style={{ color: '#94a3b8', marginBottom: '3rem', lineHeight: 1.6, fontSize: '1rem' }}>Esta ação vai apagar permanentemente o acesso, perfil e as vagas abertas de <strong>{userToDelete.nome_display || userToDelete.razao_social || userToDelete.email}</strong>. Deseja prosseguir?</p>
                        <div style={{ display: 'flex', gap: '1.5rem', justifyContent: 'center' }}>
